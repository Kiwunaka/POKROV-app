# Retained cutover readiness snapshot
...
- cutover state: `Android 1.1.6 and Windows 1.1.6 public stable-direct artifacts are runtime-synced; global cutover remains blocked on Windows trust and native Apple gates`
...
- Apple release state: `readiness only; native build, signing, device and store evidence remain manual`
- Android release state: `1.1.6 production-signed stable direct APKs are public and exact GitHub digests pass; exact 1.1.6 Huawei journey remains MANUAL_OWNER_TEST`
- Windows release state: `1.1.6 unsigned stable-direct setup and portable artifacts are public and exact GitHub digests pass; trusted signing and clean TUN/DNS remain manual gates`
- Windows 1.2.0 working-source state: `service-first Core/installer cutover is
  locally proved; durable journal, exact owned-adapter snapshot/restore and
  recovery fault paths plus bounded secret-free SCM/IPC/network/power
  breadcrumbs are locally proved; portable ZIP is ineligible; SCM
  install, live crash/reboot/route/DNS recovery, clean-VM traffic/DNS, signing
  and exact-candidate gates remain open`
- Android 1.2.0 working-source state: `private generic foreground/lockscreen
  notification and one-way removal of legacy country/route/speed preferences
  are locally proved; managed TUN MTU accepts 1280..1500, defaults to 1280 and
  is capped by a usable platform-interface ceiling; a generation-fenced parent
  scope now owns VPN-session work, Activity-scoped raw threads are removed, and
  live rates/totals come from Core CommandStatus TUN counters with explicit
  unavailable/warming/reset/overflow handling; direct/store product flavors now
  separate install authority, store merged manifest/APK has no direct installer
  permission/provider/code path, and direct cache/intent handoff verifies full
  APK and installed-signer continuity; physical process-death, endurance,
  OEM/lockscreen, production signing, Play console, release-build and
  exact-candidate gates remain open`
- Android and Windows engineering verification: `the 1.1.6 release seed records exact eight-asset GitHub digest and runtime catalog checks; it does not upgrade retained 1.0.2 device evidence into exact 1.1.6 device proof`
- public store readiness: `not approved`
- public cutover approval: `false; Android stable-direct is accepted, while Windows trusted-signing and native Apple gates block the stronger global claim`
- public Android release approval: `true for exact 1.1.6 stable-direct artifacts`
- public Android release blockers: `none for the recorded stable-direct artifact publication; offline signing recovery, endurance and exact-device checks remain operational/manual follow-ups; Play is NOT_REQUESTED`
- public Windows release approval: `false for stronger trusted/global cutover claims`
- public Windows release blockers: `exact machine-wide installer SCM/owner
  matrix, live crash/reboot/uninstall recovery and route/DNS restoration,
  trusted signing, uploaded exact-candidate live smoke and clean TUN/DNS
  evidence`
...
## Release-Handoff v2 Boundary

The current `config/release-handoff.seed.json` remains the retained 1.1.6
schema-v1 publication record. It is migration input, not authority for a new
candidate. Every new 1.2.0 candidate must use strict schema v2 and bind exact
platform, client, Core and release-index revisions; shell/Core compatibility;
artifact digests; supply evidence; manual gates; and same-byte promotion.

`scripts/new-release-handoff-v2.ps1` generates this contract outside retained
release artifacts and requires the platform offline validator to accept it.
The synthetic contract test creates no candidate and carries no signing,
device, RU-origin, runtime-sync or promotion claim.

The client PR/push workflow checks out platform `master` and Core `main`, then
runs `validate-seed.ps1` with explicit roots. Platform release-bound workflows
and Core CI run the same contract from their triggering revisions. A missing
checkout, parity mismatch, generator failure, or platform v2 rejection fails
the job. These checks create no retained candidate and do not prove signing,
device, public-origin, deploy, or promotion readiness.
...
- Build the outside-store candidate with `--flavor direct`; build any Play
  candidate with `--flavor store`. Confirm the exact merged store manifest has
  no `REQUEST_INSTALL_PACKAGES` or `FileProvider`, and confirm direct/store keep
  the same package ID and approved signing lineage.
- Exercise a same-signer upgrade plus the package mismatch, signer mismatch,
  downgrade, incompatible SDK/ABI, malformed APK and repeated-cache negative
  paths on the exact release candidate. Source/JVM proof does not replace this
  device gate.
