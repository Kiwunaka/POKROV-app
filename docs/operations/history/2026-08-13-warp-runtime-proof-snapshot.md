# Retained WARP runtime proof snapshot

Document class: `EVIDENCE`
Snapshot date: 2026-08-13

This file preserves the exact WARP readiness text that existed before
`WO-003G` separated the current `1.2.0` gate from candidate-specific history.
Every status below is evidence for its named build. It is not the current
release decision and cannot authorize a new candidate or promotion.
