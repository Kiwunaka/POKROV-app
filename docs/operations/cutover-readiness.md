...
| AWG lifecycle and direct DoH lab | `PASS_SUPPORTING_SUPERSEDED_CORE_AWG2_AWG31; PASS_DIRECT_DOH_CELLULAR; ACTIVE_CORE_PHYSICAL_REPEAT_OPEN` | The owned server was multi-addressed and selected the wrong reply source. Platform correction `f79974c…ee6` adds guarded default-off source-port policy routing plus narrow final SNAT with PLAN/APPLY/readback/rollback and service-cycle recreation. Production-signed Android `1.2.0+4046` APK `930aec97…6058` from client `7a633a8…bbb` embeds Core `3c2b114…d0f`; on physical Beeline AWG2 and closed AWG3.1 each passed fresh authenticated handshakes with bidirectional inner traffic. This resolves the server diagnosis but is supporting evidence only after the active binding moved to `547f096`. Exact-active-byte physical repeat and the full device/lifecycle matrix remain open. Direct HTTPS DoH evidence is also supporting only; a dedicated resolver runtime is still missing. |
...
| Android artifact/signing | `PASS_SUPPORTING_SUPERSEDED_CORE; SECURITY_FIXED_APK_BUILD_OPEN` | Production-signed arm64 APK `930aec97…6058` from client `7a633a8…bbb` installs as `1.2.0+4046` and passed the bounded physical AWG2/AWG3.1 retest, but embeds superseded Core `3c2b114`. A new signed APK binding active Core `547f096` has not yet been built; no strict-v2 candidate exists. |
...
| Windows package/signing | `REBUILD_REQUIRED_FOR_ACTIVE_CORE`; signing `SKIPPED_BY_OWNER` | Prior setup `f12dc8da…e6fa` remains supporting evidence for superseded Core `3c2b114…d0f`. The active `547f096…8cd` DLL is exact and locally verified, but a new setup identity and manifest have not yet been produced. The owner exception and mandatory SmartScreen warning remain recorded. No strict-v2 candidate or trusted-signing claim exists. |
...
| Runtime sync | `PASS_LOCAL_PRE_CANDIDATE_BINDING; PHYSICAL_REPEAT_OPEN; PRODUCTION_NOT_MUTATED` | Exact security-fixed Core `547f096` AAR/DLL bytes are synchronized into the client worktree and validated against the machine contract. Windows proxy-only repeat passes; Android exact-byte physical repeat remains open. No candidate, deploy or production mutation occurred. |
