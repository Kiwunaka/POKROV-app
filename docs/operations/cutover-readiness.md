...
| Clean frozen revisions | `IN_PROGRESS_LOCAL_FREEZE; PROMOTION_BLOCKED` | Core `54e76bb…fa13` is clean and pushed. Client build-4044 binding and scoped platform AWG-lab scripts still require committed revisions and cross-repository preflight; release-index `32f560d…2d4a` remains unchanged. |
...
| Core replacement | `PASS_MIXED_PLATFORM_LOCAL` | Android has two byte-identical `1.1.0` AAR builds from Core `54e76bb…fa13`, including AWG DNS and Android outer-socket protection; Windows retains exact DLL bytes from `344b317…8f6`. No tag, candidate signing, platform convergence or publication is claimed. |
| AWG lifecycle and direct DoH lab | `PASS_CURRENT_ORIGIN_CORE_SERVER; PASS_4044_CONTROL_PLANE; BLOCKED_BY_NETWORK_CURRENT_BEELINE_REVERSE_UDP; PASS_DIRECT_DOH_CELLULAR; EXACT_CANDIDATE_PENDING` | Live alignment v2 passes for AWG2 and randomized-trailer AWG3.1. Build 4044 reached both listeners from physical Beeline, but guarded plain-UDP controls proved that each server received and echoed 3/3 while the phone received 0/3. That blocks AWG on this current origin without blaming crypto or global protocol behavior. Direct HTTPS DoH resolved all three bounded AI/Games queries; application traffic still uses the VPN target. Exact-candidate AWG, leak, blocked-UDP/53 and full network-matrix proof remain pending. |
| Android LDPlayer/physical source-lab rehearsal | `PASS_4044_INSTALL_SIGNER; PASS_4044_NORMAL_WARP_EGRESS; BLOCKED_4044_AWG_REVERSE_UDP; EXACT_CANDIDATE_PENDING` | Working APK `1.2.0+4044`, SHA-256 `7417191b9fab0469e2040ae535a5e51ca34821da9848e3af5a26aaf7a2f04f45`, matched the production certificate and installed on the Huawei. After lab unbind, WARP connected on Beeline and both IP and DNS+egress probes passed; runtime readback had no AWG final endpoint and DNS returned to VPN-default. These ad hoc working bytes are not a strict-v2 candidate and do not clear the full physical matrix. |
