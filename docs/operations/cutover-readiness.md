...
| Latest exact candidate | Private signed `pokrov-1.2.0-candidate.25`, app `1.2.0+4053`; signed supply and Gate F `BLOCKED 5/14/0` are current, while installed Windows/Android and the other explicit non-PASS rows remain open |
...
`BLOCKED 5/14/0`; exact installed Windows and Android are not yet proved. It
...
| Windows package/signing | `PASS_EXACT_CANDIDATE_25_SIGNED_SUPPLY_AND_11_OF_11`; signing `SKIPPED_BY_OWNER` | Setup `ffc9b07c…7fb3`, `29139238` bytes, binds the corrected build-4053 source and validates `11/11`; installed runtime remains `NOT_RUN`. SmartScreen warning remains mandatory. |
| Windows clean-host proof | `PREPARED_NOT_RUN_EXACT_CANDIDATE_25` | The private hosted gate is bound to the exact setup and will prove silent install, installed-file identity, LocalSystem service, ordinary authenticated IPC, service restart, clean uninstall and idle route/DNS restoration. Connected TUN/DNS/egress is a separate gate. |
...
| Hosted CI | `PASS_EXACT_CANDIDATE_25_ATTACHED_CHECKS` | Attached checks pass across exact platform, client, Core and release-index source SHAs. The Windows clean-host workflow is separately prepared and not yet run. |
...
2. Publish the exact setup only to the private client CI carrier and run the
   ephemeral Windows 2025 clean-host install/service/uninstall gate.
3. Run exact candidate.25 connected contention, TUN/DNS/egress, recovery and
   connected-uninstall checks in the isolated Windows VM.
4. Run Android device and the remaining Windows gates. A retained
