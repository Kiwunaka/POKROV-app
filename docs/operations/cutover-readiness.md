...
| Clean frozen revisions | `IN_PROGRESS_LOCAL_FREEZE; PROMOTION_BLOCKED_BY_ACTIONS_BILLING` | The prior 15/15 locally gated runtime tuple is Core `f44dbe8…f90d`, platform `e5ef03a…11db` and client `c196dff…1f61`. The working corrections are platform `34d1551…f435c` and client `51f41c6…dfa7b`; focused platform/client contracts plus cross-repository seed validation pass, but the exact correction tuple has not repeated every aggregate gate. Hosted PR checks remain billing-blocked; release-index `32f560d…2d4a` is unchanged. |
...
| Android build-4045 host correction | `PASS_4045_DISCONNECTED_HOST_TRUTH; FAIL_4045_PREDEPLOY_ANDROID_ACTIVATION; EXACT_CANDIDATE_PENDING` | Production-signed build 4045 cold-started honestly disconnected on Huawei and LDPlayer. A bounded physical connect control started the POKROV service but produced no Android VPN transport after about 25 seconds; post-connect UI, DNS and HTTPS were not claimed. Exact cleanup stopped POKROV, confirmed no VPN transport, restored Hiddify foreground and kept Wi-Fi disabled. The paired platform issuance correction is not deployed. |
| Candidate handoff | `MISSING` | One strict-v2 handoff binds exact revisions, artifacts and gates. |
| Android artifact/signing | `PASS_WORKING_4045_SIGNER; MISSING_EXACT_CANDIDATE` | Production-signed arm64 and x86_64 working APKs from client `51f41c6…dfa7b` pass package/version/ABI/non-debuggable/signer verification and install as `1.2.0+4045`. No strict-v2 candidate binds these bytes. |
