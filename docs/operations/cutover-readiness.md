...
| External Smart-DNS lab | `PASS_LOCAL_SOURCE; PACKAGE_AND_RESOLVER_PROOF_MISSING` | Build target 4046 accepts the mode only for custom HTTPS DoH plus direct DNS and an AI/Games purpose route, sends only those selected groups through the existing direct outbound, preserves user-rule precedence and fails closed on invalid combinations. App-shell analyze and `404/404` tests pass. No 4046 package, compatible owned resolver or live service-access/leak/lifecycle evidence exists. |
