...
| AWG current Windows-origin Core probe | `BLOCKED_BY_NETWORK_CURRENT_WINDOWS_ORIGIN_REVERSE_UDP; CLIENT_APP_NOT_EXERCISED` | Exact Core `e8eb772…41a1` emitted outer packets for AWG2 and AWG3.1 but received no outer response. Concurrent address-free AWG2 server capture counted `34` inbound and `8` outbound packets, including `8` initiation and `8` response-sized packets, with no handshake; this proves reverse-UDP loss on this origin rather than an AWG2 contract/key/server-response failure. AWG3.1 keeps the narrower `FAIL_NO_OUTER_RESPONSE_CURRENT_WINDOWS_ORIGIN` because its server capture was not repeated. The probe did not use the retained Windows DLL, service, app, TUN or DNS and changed no rollout/device binding. |
