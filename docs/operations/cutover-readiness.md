...
|---|---|---|
| Package/version parity | `PASS_LOCAL` | Android and Windows are `1.2.0+34`; the shared app-shell remains product version `1.2.0`. |
...
| AWG lifecycle and direct DoH lab | `PASS_LOCAL_SOURCE_AND_TEST; EXACT_ARTIFACT_INTEROP_PENDING` | The unchanged production Core tree passed 25 AWG2 plus 25 AWG3.1 real userspace-device lifecycle repetitions on test-only PR 5. Build 34 carries the managed-AWG profile fix plus AWG endpoint egress-probe support. Candidate.4 dropped the endpoint before tunnel start, while build 33 then fail-closed because the Android probe allowlist recognized WARP but not AWG. New exact-artifact handshake, tunnel DNS, external egress and leak proof remain pending. |
