...
| Core replacement | `PASS_SINGLE_SOURCE_LOCAL_SECURITY_FIXED` | Android and Windows now bind Core `547f096…8cd`. Two Android AAR builds are byte-identical at `7895b2f7…1a63`; two Windows DLL builds are byte-identical at `6bf2243f…c45`; both SBOMs and reproducibility evidence are pinned. The exact DLL passes 100 proxy-only start/stop cycles, and local Core verification reports zero reachable vulnerability findings after the `GO-2026-6303` fix. No tag, candidate signing or publication is claimed. |
| AWG lifecycle and direct DoH lab | `PASS_ACTIVE_CORE_TRANSPORT; FAIL_ACTIVE_CORE_SELECTED_EGRESS; PASS_DIRECT_DOH_CELLULAR` | Production-signed APK `7d1d4093…58ee5` from client `064fcd0...` embeds active Core `547f096...` and installs/readbacks exactly on physical Huawei. On Beeline, AWG2 and AWG3.1 pass fresh authenticated handshakes and bidirectional TCP/UDP payload without reset, while Android selected-endpoint verification fails `EGRESS-001` for both. Exact operator Core interop passes authenticated TLS/HTTP. IPv4-only managed DNS matches the IPv4-only route but does not fix the common Android endpoint-probe failure. Direct HTTPS DoH remains supporting only; a dedicated resolver runtime and full device/lifecycle matrix are still missing. |
| AWG current Windows-origin Core probe | `PASS_SUPPORTING_SUPERSEDED_CORE; ACTIVE_DLL_PROXY_ONLY_PASS` | After the owned server reply-source correction, Core `3c2b114` current-origin interop passed AWG2 and AWG3.1 and repeated after a service stop/start cycle. The active `547f096` client DLL is converged and passes reproducibility, ABI and proxy-only checks, but has not repeated live interop and no probe exercised the Windows app, SCM service, TUN, DNS or leak protection. |
...
| Android artifact/signing | `PASS_PRODUCTION_SIGNED_SECURITY_FIXED_PRE_CANDIDATE` | Production-signed ARM64 APK `7d1d4093…58ee5` from client `064fcd0...` installs/readbacks exactly as release/non-debuggable `1.2.0+4046`, embeds active Core `547f096...` and matches the bound AAR entry. No strict-v2 candidate exists. |
...
| Windows package/signing | `PASS_PRE_CANDIDATE_ARTIFACT`; signing `SKIPPED_BY_OWNER` | Clean client `b4c9117…9f0` plus Core `547f096…8cd` produced setup `6ef7899d…cbe9`, `28918848` bytes, with manifest `8dced605…f74b`, all eight bundle files hash-bound and exact DLL `6bf2243f…c45`. Retained evidence is `4a3bc7b9…c730`. The owner exception and mandatory SmartScreen warning are recorded. No strict-v2 candidate, clean-host proof or trusted-signing claim exists. |
...
| Hosted CI | `CORE_PASS; PLATFORM_CLIENT_BLOCKED_BY_ACCESS` | Exact Core run `33227157016` passes all five jobs. Platform/client hosted jobs stop before product steps under the owner-solo/no-purchase lane and remain non-PASS. No GitHub plan purchase or protected-branch enforcement is required. |
| Runtime sync | `PASS_LOCAL_BINDING_AND_ANDROID_TRANSPORT; ANDROID_EGRESS_FAIL; WINDOWS_LIVE_OPEN` | Exact security-fixed Core `547f096` AAR/DLL bytes are synchronized and machine-validated. Windows proxy-only repeat passes. Exact Android ordinary Frankfurt reaches verified green and AWG2/AWG3.1 transport passes on physical Beeline, but both endpoint probes remain `EGRESS-001`. No candidate or promotion occurred. |
