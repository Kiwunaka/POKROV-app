...
| Windows exact focus and synthetic runtime | `PASS_EXACT_CANDIDATE_33_BOUNDED` | Isolated Windows 11 proves candidate.32 update, exact `WIN-001`, authenticated service IPC, direct/Smart-DNS TUN/DNS lifecycles and exact cleanup. Managed-node, restart and connected-uninstall coverage remain open. |
