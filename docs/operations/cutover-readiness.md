...
| External Smart-DNS lab | `PASS_LOCAL_SOURCE; PACKAGE_AND_RESOLVER_PROOF_MISSING` | Build target 4046 accepts only HTTPS port 443 with exact `/dns-query`, no query/fragment/token, direct DNS and an AI/gaming-service purpose route. Only selected suffix DNS questions use that DoH server; normal final DNS stays unchanged. Selected service connections use the existing direct outbound, explicit user rules retain precedence and invalid state fails closed. The client/platform policy bytes match, app-shell analyze and `412/412` tests pass. The owned non-recursive DoH/SNI server remains source-only/default-off; no corrected 4046 package, dedicated node or live service-access/leak/lifecycle evidence exists. |
