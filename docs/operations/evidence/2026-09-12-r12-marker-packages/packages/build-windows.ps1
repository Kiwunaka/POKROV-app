...
foreach ($taskRow in $taskRetention.files) {
  if ((Get-FileHash -LiteralPath $taskRow.path).Hash.ToLowerInvariant() -ne $taskRow.sha256) { throw 'Previous package changed' }
  if ((Get-FileHash -LiteralPath $taskRow.retained_path).Hash.ToLowerInvariant() -ne $taskRow.sha256) { throw 'Rollback bytes mismatch' }
}
...
$taskCacheDestination=[IO.Path]::GetFullPath('E:/r12-marker-prior-flutter-cache-20260912')
if ($taskCacheSource -ne 'E:\r12client\apps\windows_shell\.dart_tool\flutter_build' -or $taskCacheDestination -ne 'E:\r12-marker-prior-flutter-cache-20260912') { throw 'Cache scope mismatch' }
