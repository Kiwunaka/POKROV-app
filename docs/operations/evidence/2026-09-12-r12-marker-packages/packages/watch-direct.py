...
assert not [p for p in psutil.process_iter(['name']) if p.info['name'].lower() in ['java.exe','virtualboxvm.exe','vboxheadless.exe','dnplayer.exe','ld9boxheadless.exe']]
floor=40*2**30
assert shutil.disk_usage('C:/').free>floor+256*2**20
assert shutil.disk_usage('E:/').free>floor+1536*2**20
started=time.time()
result={'status':'RUNNING','source_client':source,'source_core':'6b271decead88b708e2fc03984b703b0a4e63ebd','utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'floor_bytes':floor,'abort_margin_e':512*2**20,'abort_margin_c':256*2**20,'poll_seconds':0.05,'phase':'FOUR_CURRENT_APKS_AAB_PENDING_WINDOWS_ALREADY_BUILT','samples':[],'owned_java_cleanup':[]}
owned={}
last_record=0
minc=mine=2**63
with (out/'android-direct-driver.log').open('wb') as log:
    process=subprocess.Popen(['pwsh','-NoLogo','-NoProfile','-File',str(out/'build-direct-wrapper.ps1'),'-ExpectedSource',source],stdout=log,stderr=subprocess.STDOUT,creationflags=0x08000000|0x00004000)
    ...
    while True:
        ...
        if now-last_record>=1:
            ...
            (out/'android-direct-progress.json').write_text(json.dumps(result,indent=2)+'\n')
            last_record=now
        if freec<floor+256*2**20 or freee<floor+512*2**20:
