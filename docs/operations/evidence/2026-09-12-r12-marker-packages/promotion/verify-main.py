...
out=Path('C:/r12-marker-atomicity-promotion-20260912');source=json.loads((out/'client-source-promoted.json').read_bytes());merge=json.loads((out/'client-merge-binding.json').read_bytes());repo='Kiwunaka/POKROV-app'
def gh(path):return json.loads(subprocess.check_output(['gh','api','repos/'+repo+'/'+path]))
run=gh('actions/runs/34688044421')
...
if run['status']=='completed':
 ...
 jobs=gh('actions/runs/34688044421/jobs')['jobs'];assert jobs and all(j['conclusion']=='success' for j in jobs)
