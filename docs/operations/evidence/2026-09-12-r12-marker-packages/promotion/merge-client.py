...
source = json.loads((out / 'client-source-promoted.json').read_bytes())
...
pr = gh('pr', 'view', '125', '--repo', repo, '--json', 'headRefOid,baseRefName,state,isDraft')
...
assert gh('api', f'repos/{repo}/branches/main/protection') == json.loads((out / 'client-protection-before.json').read_bytes())
if pr['isDraft']:
    subprocess.run(['gh', 'pr', 'ready', '125', '--repo', repo], cwd=root, check=True)
subprocess.run(['gh', 'pr', 'merge', '125', '--repo', repo, '--squash', '--match-head-commit', source['signed_commit']], cwd=root, check=True)
pr = gh('pr', 'view', '125', '--repo', repo, '--json', 'state,mergedAt,mergeCommit,headRefOid,url')
