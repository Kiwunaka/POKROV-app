...
import re, base64, datetime as dt, hashlib, json, logging, os, pathlib, sys, urllib.error, urllib.parse, urllib.request
...
CIPHER_SHA = '176810f528308df47b02fd60421d714024baa7ee50c0c1f9d0d4abae45d0463a'
...
report = {'status': 'UNCONFIRMED', 'case_number': CASE, 'origin': 'brain-origin real HTTPS and worker-key in-memory scan', 'source_revision': SOURCE, 'external_oidc_retested': False, 'planted_native_to_app_canaries_exercised': True, 'plaintext_or_credentials_exported': False}
def now(): return dt.datetime.now(dt.timezone.utc).replace(tzinfo=None)
CANARY_NEEDLES = ['/data/user/0/synthetic/R12_ANDROID_PATH_2c_20260912.conf', '198.51.100.127', 'Authorization: Bearer R12_ANDROID_AUTH_2c_20260912', 'Cookie: session=R12_ANDROID_COOKIE_2c_20260912', 'R12_ANDROID_AUTH_2c_20260912', 'R12_ANDROID_CONFIG_2c_20260912', 'R12_ANDROID_COOKIE_2c_20260912', 'R12_ANDROID_CREDENTIAL_2c_20260912', 'R12_ANDROID_PATH_2c_20260912', 'R12_ANDROID_PII_2c_20260912', 'R12_ANDROID_PROVIDER_2c_20260912', 'R12_ANDROID_URL_2c_20260912', 'config={private_key:R12_ANDROID_CONFIG_2c_20260912}', 'https://example.invalid/R12_ANDROID_URL_2c_20260912?secret=synthetic', 'password=R12_ANDROID_CREDENTIAL_2c_20260912', 'provider_payload=R12_ANDROID_PROVIDER_2c_20260912', 'qa+R12_ANDROID_PII_2c_20260912@example.invalid']
def contains_canary(raw): return any(marker.encode() in raw for marker in CANARY_NEEDLES)
def guard(s):
    ticket = s.get(SupportTicket, CASE)
    assert ticket and ticket.created_at >= dt.datetime(2026, 9, 12, 7, 46)
    ...
    assert row.build_number == '4053'
    assert row.status == 'validated' and row.expected_sha256 == CIPHER_SHA and row.diagnostic_profile == 'extended'
    return ticket, row
...
try:
    ...
    detail = read('/support/tickets/' + str(CASE)); bundles = [b for b in detail['support_bundles'] if b['diagnostic_profile'] == 'extended']; assert len(bundles) == 1
    bundle = bundles[0]; assert bundle['status'] == 'validated' and bundle['diagnostic_profile'] == 'extended'
    ...
    for item in payload['files']:
        ...
        if item['path'] == 'build/identity.json':
            ...
            report['build_identity'] = {k: ident.get(k) for k in ['app_version','build_id','platform','channel']}
            assert ident['build_id'] == '4053' and ident['channel'] == 'direct'
        if item['path'] == 'events/recent.jsonl':
            events=[json.loads(line) for line in content.splitlines()];report['events_count']=len(events);assert events
        if item['path'] == 'network/summary.json':
        ...
        if item['path'] == 'system/summary.json':
            report['system_keys']=sorted(json.loads(content))
        files.append({'path': item['path'], 'bytes': len(content), 'sha256': hashlib.sha256(content).hexdigest(), 'canary_matches': 0})
    assert {f['path'] for f in files} == {'build/identity.json', 'network/summary.json', 'redaction/report.json', 'system/summary.json'}
    report['payload_scan'] = {'profile': payload['manifest']['profile'], 'plaintext_bytes': len(plaintext), 'plaintext_sha256': hashlib.sha256(plaintext).hexdigest(), 'file_count': len(files), 'files': files, 'canary_matches': 0, 'extended_included': True, 'crash_included': False}
    with SessionLocal() as s:
        ticket, _ = guard(s)
    ...
    report['native_canary_categories']=9; report['marker_needles_scanned']=len(CANARY_NEEDLES)
    report['status'] = 'PASS_ANDROID_EXTENDED_ACCESS_SCAN'
except BaseException as error:
    report['status'] = 'FAIL_NATIVE_EXTENDED_SCAN'; report['failure_type'] = type(error).__name__
...
sys.exit(0 if report['status'] == 'PASS_ANDROID_EXTENDED_ACCESS_SCAN' else 1)
