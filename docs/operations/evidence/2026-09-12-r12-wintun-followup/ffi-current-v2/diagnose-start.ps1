...
$start.Arguments='-NoProfile -ExecutionPolicy Bypass -File C:/r12-c05-wintun-20260912/ffi-current-v2/child.ps1'
...
if($process.ExitCode -ne 0){[ordered]@{exit_code=$process.ExitCode;sinks=$sinks}|ConvertTo-Json -Depth 6|Set-Content -LiteralPath "$root/subprocess-failure.json" -Encoding UTF8;[IO.File]::WriteAllText("$root/controlled-harness-start-error.log",$streams.stderr);throw 'FFI subprocess failed; controlled startup diagnostic retained'}
