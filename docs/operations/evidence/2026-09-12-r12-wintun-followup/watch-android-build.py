...
result={'status':'RUNNING','source':source,'core':'880bff65ad665844828fe50fc395e9cfc1cd81b4','utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'floor_bytes':floor,'abort_margin':64*2**20,'estimated_incremental_android_budget_bytes':1024*2**20,'prior_outputs_retained':True,'skipped_build_script_checks':'Focused current runtime81 Android8 seed/docs already passed; full CI and native CTest separate','samples':[]}
with (out/'android-driver.log').open('wb') as log:
    process=subprocess.Popen(['pwsh','-NoLogo','-NoProfile','-File',str(out/'build-android.ps1'),'-ExpectedSource',source],stdout=log,stderr=subprocess.STDOUT,creationflags=0x08000000|0x00004000)
    ...
    while True:
        ...
        if min(row['free_c'],row['free_e'])<floor+64*2**20:
        ...
        (out/'android-build-progress.json').write_text(json.dumps(result,indent=2)+'\n')
        time.sleep(3)
(out/'android-build-progress.json').write_text(json.dumps(result,indent=2)+'\n')
