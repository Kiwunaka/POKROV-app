...
pr = gh('pr', 'view', '119', '--repo', repo, '--json', 'headRefOid,baseRefName,state,isDraft')
...
if pr['isDraft']:
    subprocess.run(['gh', 'pr', 'ready', '119', '--repo', repo], cwd=root, check=True)
subprocess.run(['gh', 'pr', 'merge', '119', '--repo', repo, '--squash', '--match-head-commit', source['signed_commit']], cwd=root, check=True)
pr = gh('pr', 'view', '119', '--repo', repo, '--json', 'state,mergedAt,mergeCommit,headRefOid,url')
