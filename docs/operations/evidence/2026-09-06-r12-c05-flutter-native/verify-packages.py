...
OUT = Path('E:/r12-c05-flutter-native')
OLD = Path('E:/r12-c05-header-review')
...
for apk in apks:
    ...
    with zipfile.ZipFile(path) as package:
        ...
        flutter_libs = {n: sha(package.read(n)) for n in package.namelist() if n.endswith('/libflutter.so')}
        assert flutter_libs
        for name in flutter_libs:
            abi = name.split('/')[1]
            assert flutter_libs[name] == sha((OUT / 'stripped' / (abi + '.so')).read_bytes())
        flutter_notices_sha = sha(package.read('assets/flutter_assets/NOTICES.Z'))
        with zipfile.ZipFile(OLD / 'android' / path.name) as previous_apk:
            assert package.read('assets/flutter_assets/NOTICES.Z') == previous_apk.read('assets/flutter_assets/NOTICES.Z')
    ...
    android.append({'path': str(path), 'bytes': path.stat().st_size, 'sha256': sha(path.read_bytes()), 'notice_sha256': sha(notice), 'core_entries': libs, 'flutter_engine_entries': flutter_libs, 'flutter_generated_notices_sha256': flutter_notices_sha, 'signing': {'command': command, 'exit_code': 0, 'signer': 'Android Debug', 'certificate_sha256': certificate}})
