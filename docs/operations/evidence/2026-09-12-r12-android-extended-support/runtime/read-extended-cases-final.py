...
with SessionLocal() as s:
 ...
 own_cases=s.query(SupportTicket).filter(SupportTicket.user_tg_id==t.user_tg_id,SupportTicket.created_at>=dt.datetime(2026,9,12,7,6)).all();assert sorted(x.id for x in own_cases)==[60,61,62,63]
...
'''
p=subprocess.run(['ssh.exe','-o','BatchMode=yes','-o','StrictHostKeyChecking=yes','pokrov-brain','/root/portal_bot/venv/bin/python -c '+shlex.quote(code)],capture_output=True,timeout=45);assert p.returncode==0,{'exit':p.returncode,'stderr_sha256':hashlib.sha256(p.stderr).hexdigest()};r=json.loads(p.stdout);(out/'extended-cases-final-readback.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
