...
ui.run('shell','uiautomator','dump','/data/local/tmp/r12-empty-support-code.xml');t=ui.ET.fromstring(ui.run('shell','cat','/data/local/tmp/r12-empty-support-code.xml'));fields=[n for n in t.iter('node') if n.get('class')=='android.widget.EditText' and n.get('enabled')=='true'];assert len(fields)==1
v=list(map(int,re.findall(r'\d+',fields[0].get('bounds'))));ui.run('shell','input','tap',str((v[0]+v[2])//2),str((v[1]+v[3])//2))
remote='/root/portal-r12-evidence/psm-runtime-20260910/android-policy-private-20260912-v2.json'
...
(out/'code-replay-entry.json').write_text(json.dumps(result,indent=2)+'\n');print(json.dumps(result))
