...
pr = gh('pr', 'view', '123', '--repo', repo, '--json', 'headRefOid,baseRefName,state,isDraft')
...
if pr['isDraft']:
    subprocess.run(['gh', 'pr', 'ready', '123', '--repo', repo], cwd=root, check=True)
subprocess.run(['gh', 'pr', 'merge', '123', '--repo', repo, '--squash', '--match-head-commit', source['signed_commit']], cwd=root, check=True)
pr = gh('pr', 'view', '123', '--repo', repo, '--json', 'state,mergedAt,mergeCommit,headRefOid,url')
