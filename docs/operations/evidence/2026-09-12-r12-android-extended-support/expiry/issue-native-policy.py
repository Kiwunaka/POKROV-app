...
prefix=prefix.replace("'ttl_minutes':5", "'ttl_minutes':5")
prefix=prefix.replace("audit_action='session.r12_psm_preflight'", "audit_action='session.r12_psm_native'")
prefix=prefix.replace("try:\n with SessionLocal()", "private_state=pathlib.Path('/root/portal-r12-evidence/psm-runtime-20260910/android-policy-private-20260912-v3.json');assert not private_state.exists()\ntry:\n with SessionLocal()")
body=r'''
