import hashlib,json,os,pathlib,platform,signal,socket,subprocess,time,threading,collections
assert os.getuid()==0 and platform.node()=='pokrov-r12-l04-clean'
r=pathlib.Path('/home/pokrovqa/acceptance-inputs');out=r/'candidate8-de-wire-socket-observation.json';assert not out.exists()
...
def call(action,payload=None):
 with socket.socket(socket.AF_UNIX) as s:
  s.settimeout(130);s.connect('/run/pokrov/pokrov-linuxd.sock');s.sendall((json.dumps({'protocol':'pokrov-linuxd-v1','request_id':'l04-de-recheck8-'+action,'action':action,'payload':payload or {}})+'\n').encode());v=json.loads(s.makefile('rb').readline(65537))
...
visit(final,[]);choices=[x for x in choices if hashlib.sha256(x['server'].encode()).hexdigest()=='9cbe63494f2822fa475d6f2c0d71e9f22e05e841811d12c0472c632f50a5b7a7'];assert len(choices)==1
resolved=socket.getaddrinfo(choices[0]['server'],443,type=socket.SOCK_STREAM);assert any(hashlib.sha256(x[4][0].encode()).hexdigest()=='66624e15725d946c4321da789698aff1d98a64de1af16bb4dc43f2bff20bea67' for x in resolved)
...
def probe(label,selected=None):
 ...
 for attempt in range(1):
  for target,url in [('app','https://app.pokrov.space/'),('marker','https://app.pokrov.space/api/public/authenticated-egress-probe')]:
   started=time.monotonic();stop=threading.Event();samples=[]
   def observe():
    last=None
    while not stop.is_set():
     q=subprocess.run(['ss','-Hntp'],capture_output=True,text=True,timeout=2);states=collections.Counter()
     for line in q.stdout.splitlines():
      if 'pid='+str(corepid)+',' in line and any(addr+':443' in line for addr in addresses):states[line.split()[0]]+=1
     current=dict(sorted(states.items()))
     if current!=last:samples.append({'seconds':round(time.monotonic()-started,2),'states':current});last=current
     stop.wait(.2)
   worker=None
   if selected:
    corepid=int(subprocess.check_output(['pgrep','-x','pokrov-core'],text=True).strip());addresses={x[4][0] for x in resolved if x[0]==socket.AF_INET};worker=threading.Thread(target=observe);worker.start()
   p=subprocess.run(['curl','-4','--silent','--show-error','--max-time','12','--output','/dev/null','--write-out','%{json}',url],capture_output=True,text=True)
   stop.set()
   if worker:worker.join(timeout=3);assert not worker.is_alive()
   try:v=json.loads(p.stdout)
   except ValueError:v={}
   observation['requests'].append({'attempt':attempt+1,'target':target,'core_de_socket_states':samples,'curl_exit':p.returncode,'http_code':v.get('http_code'),'seconds':round(time.monotonic()-started,2),'peer_sha256':hashlib.sha256(v.get('remote_ip','').encode()).hexdigest(),'ssl_verify_result':v.get('ssl_verify_result'),'error_kinds':[k for k in ['SSL_ERROR_SYSCALL','Connection reset by peer','unexpected eof','wrong version number','certificate','timed out','Could not resolve host','Failed to connect'] if k.lower() in p.stderr.lower()]})
