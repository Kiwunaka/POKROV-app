...
import ctypes as c
import hashlib, json, socket, struct, threading, time, runpy, os
assert 'GOMAXPROCS' not in os.environ
handle_types = runpy.run_path(str(Path(__file__).parent / 'handle-types.py'))['handle_types']

root = Path(__file__).parent
dll = Path('E:/r12-c05-utls-2662-artifacts/windows-a/pokrov-core.dll')
assert hashlib.sha256(dll.read_bytes()).hexdigest() == 'e77cc0ab979becc635ec578b8b44248b1146dd96ba34cd4de72e62130089ce2c'
fixture = Path('E:/r12-c05-utls-2662-artifacts/artifact-loopback-fixture')
fixture.mkdir()
...
for index in range(300):
    ...
    cancelled(new)
    time.sleep(0.02)
    samples.append({'cycle': index + 1, 'handles': handles(), 'types': handle_types()})
time.sleep(1)
final = handles()
last = [x['handles'] for x in samples[-20:]]
receipt = {'status': 'PASS_BOUNDED' if max(last) - min(last) <= 2 and final <= max(last) else 'FAIL_RESOURCE_STABILITY', 'dll_sha256': hashlib.sha256(dll.read_bytes()).hexdigest(), 'scheduler': 'Go default; GOMAXPROCS unset', 'cycles': 300, 'start_restart_stop': True, 'loopback_sessions_cancelled': 600, 'network_scope': '127.0.0.1 only; no TUN or host route changes', 'samples': samples, 'last_twenty_handle_range': [min(last), max(last)], 'final_handles': final, 'goroutine_measurement': 'not exported by ABI; covered by separate source race test'}
(root / 'artifact-lifecycle-2662.json').write_text(json.dumps(receipt, indent=2) + '\n', encoding='utf8')
origin.close()
