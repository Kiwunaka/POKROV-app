...
$start.Arguments='-NoProfile -ExecutionPolicy Bypass -File C:/r12-c05-setup-privacy-20260912/V01_CANARY_SETUP_PATH/child.ps1'
...
$result=[ordered]@{utc=[DateTime]::UtcNow.ToString('o');status=if($leaked){'FAIL_PRIVACY'}else{'PASS_BOUNDED_SOURCE_DLL_FFI'};origin='current-origin host isolated subprocess; invalid config before service startup; no TUN';client_source='NOT_YET_BOUND_TO_CLIENT';core_source='6b271decead88b708e2fc03984b703b0a4e63ebd';source_dll_path='C:/r12corec02/dist/windows/pokrov-core.dll';child=$child;sinks=$sinks;raw_streams_retained=$false;host_network_changed=$false;live_profile_used=$false}
