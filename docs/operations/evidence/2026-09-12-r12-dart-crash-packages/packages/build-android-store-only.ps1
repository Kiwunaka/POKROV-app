...
. (Join-Path 'E:/r12client/scripts' 'support-signing-pin.ps1')
$supportSigningPin = Resolve-PokrovSupportSigningPin `
  -RepositoryRoot (Split-Path -Parent 'E:/r12client/scripts') `
...
$repoRoot = Split-Path -Parent 'E:/r12client/scripts'
& (Join-Path 'E:/r12client/scripts' "check-client-version-parity.ps1")
...
try {
  ...
  try {
    ...
    $resumeProof=[IO.File]::ReadAllText('C:/r12-dart-crash-packages-20260912/direct-apks-verified.json') | ConvertFrom-Json
    ...
    Write-Host 'STORE-ONLY RESUME: reusing four exact verified APKs.'
