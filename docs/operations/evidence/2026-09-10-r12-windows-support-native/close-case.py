...
with SessionLocal() as s:
 ...
 assert b.status=='validated' and b.expected_sha256=='f2bcc4d5bb352e7c5a4f1926b30bad4a5bb08d2932ab75c50d9f0a024d7e5d57'
 assert t.status=='open'
 t.status='closed';t.closed_at=dt.datetime.now(dt.timezone.utc).replace(tzinfo=None);t.updated_at=t.closed_at;s.commit()
 r={'status':'PASS_OWNED_FIXTURE_CLEANUP','case_number':51,'utc':dt.datetime.now(dt.timezone.utc).isoformat(),'closed':t.status=='closed','mode':'direct isolated-fixture cleanup; not operator action acceptance','ciphertext_and_messages_retained':True,'retention_clock_unchanged':True,'external_message_sent':False,'raw_identifiers_exported':False}
 print(json.dumps(r))
'''
...
r=json.loads(p.stdout);(out/'native-case-cleanup.json').write_text(json.dumps(r,indent=2)+'\n');print(json.dumps(r))
