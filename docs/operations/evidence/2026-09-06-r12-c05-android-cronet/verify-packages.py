...
out=Path('E:/r12-c05-android-cronet');root=Path('E:/r12client');old=Path('E:/r12-c05-maven-notices/android');sha=lambda b:hashlib.sha256(b).hexdigest();r=json.loads((out/'assembly.json').read_text());asset=root/r['asset']['path'];notice=asset.read_bytes();assert sha(notice)==r['asset']['sha256'];records=[]
for item in r['sections']:assert sha(notice[item['body_offset']:item['body_offset']+item['bytes']])==item['sha256']
...
for p in sorted(apks):
 ...
 with zipfile.ZipFile(dest) as z,zipfile.ZipFile(old/p.name) as previous:
  name='assets/flutter_assets/assets/licenses/cronet-NOTICES.txt';assert z.read(name)==notice
