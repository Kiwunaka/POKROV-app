...
assert free['C:/']>floor+512*2**20 and free['E:/']>floor+2**30 and psutil.virtual_memory().available>8*2**30
other={str(p):p.stat().st_size for i in range(3) for p in Path(f'E:/LDPlayer/LDPlayer14/vms/leidian{i}').glob('*.vdi')}
r={'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'instance_index':3,'before':before,'disk_bytes_before':base,'entry_margin_e_bytes':2**30,'quit_margin_e_bytes':512*2**20,'growth_budget_bytes':512*2**20,'quit_at_growth_bytes':384*2**20,'floor_bytes':floor,'free_before':free,'other_disk_sizes_before':other,'samples':[]}
...
while True:
    ...
    if not quitting and (growth>384*2**20 or elapsed>480 or free['E:/']<floor+512*2**20 or free['C:/']<floor+256*2**20):
