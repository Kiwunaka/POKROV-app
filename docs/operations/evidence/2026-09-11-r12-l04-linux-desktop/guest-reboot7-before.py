...
r=pathlib.Path('/home/pokrovqa/acceptance-inputs');out=r/'candidate7-reboot.json';assert not out.exists()
...
def call(action):
 with socket.socket(socket.AF_UNIX) as s:
  s.settimeout(130);s.connect('/run/pokrov/pokrov-linuxd.sock');s.sendall((json.dumps({'protocol':'pokrov-linuxd-v1','request_id':'l04-reboot7-'+action,'action':action,'payload':{}})+'\n').encode());v=json.loads(s.makefile('rb').readline(65537))
...
result['network_before']=before;result['boot_id_before']=pathlib.Path('/proc/sys/kernel/random/boot_id').read_text().strip();save()
digest=r/'reboot7-profile-digest';assert not digest.exists();digest.write_text(profile_before);digest.chmod(0o600)
assert call('connect')['ok'];traffic('before_reboot_http')
result['connected_before_reboot']=pathlib.Path('/sys/class/net/pokrov0').exists();result['durable_journal_present']=pathlib.Path('/var/lib/pokrov/network-recovery.json').exists();assert result['connected_before_reboot'] and result['durable_journal_present'];save()
subprocess.run(['systemctl','reboot'],check=True,capture_output=True)
