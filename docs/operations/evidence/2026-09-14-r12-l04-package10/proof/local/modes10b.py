...
r=pathlib.Path('/home/pokrovqa/acceptance-inputs');out=r/'candidate10b-gui-route-20260914.json';assert not out.exists()
...
backup=r/'private-gui-route10b-original-20260914.json';assert not backup.exists();backup.write_bytes(profile.read_bytes());backup.chmod(0o600)
...
try:
 while time.time()-result['started_epoch']<1200:
  ...
  if snap['phase']=='running' and mode in ['fullTunnel','allExceptRu'] and mode not in seen:
   ...
   private=r/('private-gui-route10b-'+mode+'-20260914.json');assert not private.exists();private.write_bytes(raw);private.chmod(0o600)
   def core_sockets():
