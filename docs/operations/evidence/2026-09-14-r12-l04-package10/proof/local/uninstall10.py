...
r=pathlib.Path('/home/pokrovqa/acceptance-inputs');out=r/'candidate10-connected-uninstall.json';assert not out.exists()
def state():
 with socket.socket(socket.AF_UNIX) as s:
  s.settimeout(5);s.connect('/run/pokrov/pokrov-linuxd.sock');s.sendall(b'{"protocol":"pokrov-linuxd-v1","request_id":"l04-uninstall10","action":"status","payload":{}}\n');return json.loads(s.makefile('rb').readline(65537)).get('snapshot',{})
result={'operation':'connected_uninstall_and_purge','source_version':'1.2.0~beta.30-10','started_epoch':time.time(),'running_observed':False}
...
result['connected_health']=state();save();time.sleep(5)
...
for attempt in range(1):
 for label,url,expected in [('app','https://app.pokrov.space/','200'),('marker','https://api.pokrov.space/api/public/authenticated-egress-probe','204')]:
...
with (r/'candidate10-connected-uninstall.log').open('w') as log:
