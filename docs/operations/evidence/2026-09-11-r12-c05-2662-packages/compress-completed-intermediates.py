...
out=Path('C:/r12-c05-packages-20260911');root=Path('E:/r12client/apps/android_shell/build/app/intermediates/merged_native_libs').resolve()
assert json.loads((out/'build-progress-retry.json').read_bytes())['status']=='PASS'
...
(out/'completed-intermediates-compression-plan.json').write_text(json.dumps(rows,indent=2)+'\n')
for row in rows:
...
(out/'completed-intermediates-compression.json').write_text(json.dumps(receipt,indent=2)+'\n');print(json.dumps({k:v for k,v in receipt.items() if k!='files'}))
