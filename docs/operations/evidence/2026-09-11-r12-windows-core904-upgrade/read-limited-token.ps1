...
$r=[ordered]@{utc=[DateTime]::UtcNow.ToString('o');ui_pid=$ui[0].Id;ui_session=$ui[0].SessionId;ui_elevated=$elevated;service_state=$service.State;service_account=$service.StartName;service_acl_sha256=$aclHash;service_acl_preserved=($aclHash -eq $before.service_acl_sha256);service_broad_group_mutation_allowed=$publicWrite;ui_loaded_core=@($ui[0].Modules|Where-Object ModuleName -eq 'pokrov-core.dll').Count -gt 0;status=(& 'C:/Program Files/POKROV/r12_managed_probe.exe' status|ConvertFrom-Json)}
$r|ConvertTo-Json -Depth 6|Set-Content -LiteralPath 'C:/Users/Public/R12Upgrade90420260911/limited-token.json' -Encoding UTF8
if($elevated -or $publicWrite -or -not $r.service_acl_preserved -or $r.ui_loaded_core -or -not($r.status.running -and $r.status.egress_validated -and $r.status.effective_matches_staged)){throw 'Limited UI/service boundary failed'}
