...
expected_core = {abi: sha((out / 'native' / f'libpokrov-core-{abi}.so').read_bytes()) for abi in flutter}
rows = []
for p in files:
...
dex = (out / 'android-dex-packages.log').read_text(encoding='utf-8-sig')
...
result = {'schema': 'pokrov.r12.final-android-package-audit/v1',
          ...
          'dex_namespace_check': {'patterns': patterns, 'matches': matches, 'log': ref(out / 'android-dex-packages.log')},
