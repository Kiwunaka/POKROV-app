...
assert not set(before) - set(files)
embedded = sorted(set(files) - set(before))
assert len(embedded) == 6 and all(files[key]['kinds'] == {'EmbedFiles'} for key in embedded)
changed = [key for key in before if files[key]['sha256'] != before[key]['sha256']]
...
assert source_files == old_matches['selected_files']
...
for binary in scan['binary_scans']:
...
assert {identifier for row in scan['binary_scans'] for identifier in row['advisories']} == {x['id'] for x in matrix}
