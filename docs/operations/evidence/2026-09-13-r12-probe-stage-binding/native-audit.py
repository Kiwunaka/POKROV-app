...
source = work / 'source'
baseline = base / 'setup-privacy-final-6b271de-20260912'
...
result = {'status': 'RUNNING', 'source_commit': progress['source_commit'], 'source_tree': progress['source_tree'], 'baseline_source': '6b271decead88b708e2fc03984b703b0a4e63ebd', 'binaries': [], 'errors': []}
...
for name, path in files:
    ...
    union.update(deps)
    result['binaries'].append({'file': path.name, 'bytes': path.stat().st_size, 'sha256': sha(path), 'build_info': name, 'build_info_sha256': sha(audit / name), 'module_count': len(deps), 'same_dependency_versions_and_replacements': True})
