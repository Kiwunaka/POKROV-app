...
CIPHER_SHA = 'a33b11cc8d38e396fcfb0c34b8fcb475929a5d7ce056f4e9a3aa9da52ecfbde5'
...
report = {'status': 'UNCONFIRMED', 'case_number': CASE, 'origin': 'brain-origin real HTTPS and worker-key in-memory scan', 'source_revision': SOURCE, 'external_oidc_retested': False, 'plaintext_or_credentials_exported': False}
...
def guard(s):
    ...
    assert ticket and ticket.created_at >= dt.datetime(2026, 9, 10, 14, 7)
...
try:
    ...
    report['status'] = 'PASS_NATIVE_SUMMARY_CANARY_SCAN'
...
sys.exit(0 if report['status'] == 'PASS_NATIVE_SUMMARY_CANARY_SCAN' else 1)
