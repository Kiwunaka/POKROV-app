...
r={'utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'vm':vm,'snapshots_retained':len(ids),'nic':'none','leaf':str(leaf),'leaf_before_bytes':baseline,'growth_budget_bytes':2**30,'shutdown_at_growth_bytes':400*2**20,'pause_at_growth_bytes':700*2**20,'floor_bytes':floor,'free_before':free,'host_network_mutation':'NONE','budget_usd':0,'samples':[]}
(out/'canary-vm-before.json').write_text(json.dumps(r,indent=2)+'\n')
with (out/'canary-vm-start.log').open('wb') as log:subprocess.run([v,'startvm',vm,'--type','separate'],stdout=log,stderr=subprocess.STDOUT,check=True)
...
while True:
    ...
    if not paused and (growth>=700*2**20 or free['E:/']<floor+768*2**20 or free['C:/']<floor+256*2**20):
        ...
        if paused:break
    if not shutdown and (growth>=400*2**20 or elapsed>=480):
    ...
    if elapsed-last>=5:
        ...
        (out/'canary-vm-progress.json').write_text(json.dumps(r,indent=2)+'\n')
...
(out/'canary-vm-result.json').write_text(json.dumps(r,indent=2)+'\n')
