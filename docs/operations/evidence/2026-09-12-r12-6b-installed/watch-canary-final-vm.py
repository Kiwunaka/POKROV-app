...
(out/'canary-final-vm-before.json').write_text(json.dumps(r,indent=2)+'\n')
subprocess.run([v,'modifyvm',vm,'--nic1','bridged','--bridgeadapter1','Intel(R) Ethernet Controller (3) I225-V'],check=True)
with (out/'canary-final-vm-start.log').open('wb') as log:subprocess.run([v,'startvm',vm,'--type','separate'],stdout=log,stderr=subprocess.STDOUT,check=True)
...
while True:
    ...
    if elapsed-last>=5:
        ...
        (out/'canary-final-vm-progress.json').write_text(json.dumps(r,indent=2)+'\n')
...
if not paused:
    ...
    r['nic_restored']='none'
(out/'canary-final-vm-result.json').write_text(json.dumps(r,indent=2)+'\n')
