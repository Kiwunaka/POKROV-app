...
. (Join-Path 'C:/r12-current-windows-source-20260911/scripts' 'support-signing-pin.ps1')
$supportSigningPin = Resolve-PokrovSupportSigningPin `
  -RepositoryRoot 'C:/r12-current-windows-source-20260911' `
...
$repoRoot = 'C:/r12-current-windows-source-20260911'
& (Join-Path 'C:/r12-current-windows-source-20260911/scripts' "check-client-version-parity.ps1")
...
try {
  ...
  try {
    ...
    if ($LASTEXITCODE -ne 0) {
      throw "Flutter scoped ARM64 APK build failed with exit code $LASTEXITCODE."
    }
...
$artifacts = @(
  [ordered]@{path = Join-Path $outputDirectory "app-direct-release.apk"; abi = "arm64-v8a"}
)
