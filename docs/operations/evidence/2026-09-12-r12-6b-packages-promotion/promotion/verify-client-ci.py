...
r=pathlib.Path('C:/r12-c05-setup-privacy-20260912/promotion');source=json.loads((r/'client-source-promoted.json').read_bytes());sha=source['signed_commit'];repo='Kiwunaka/POKROV-app'
def gh(path):return json.loads(subprocess.check_output(['gh','api',f'repos/{repo}/'+path]))
run=gh('actions/runs/34666141729');assert run['head_sha']==sha and run['status']=='completed' and run['conclusion']=='success'
...
p=r/'client-ci-pr.json';assert not p.exists();p.write_text(json.dumps(receipt,indent=2)+'\n');print(json.dumps({k:v for k,v in receipt.items() if k!='runs'}))
