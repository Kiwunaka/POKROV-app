...
pr = gh('pr', 'view', '15', '--repo', repo, '--json', 'headRefOid,baseRefName,state,isDraft')
...
if pr['isDraft']:
    subprocess.run(['gh', 'pr', 'ready', '15', '--repo', repo], cwd=root, check=True)
subprocess.run(['gh', 'pr', 'merge', '15', '--repo', repo, '--squash', '--match-head-commit', source['signed_commit']], cwd=root, check=True)
pr = gh('pr', 'view', '15', '--repo', repo, '--json', 'state,mergedAt,mergeCommit,headRefOid,url')
