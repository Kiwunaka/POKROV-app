...
for label,repo,run_id in [('core','Kiwunaka/pokrov-core',34667366713),('client','Kiwunaka/POKROV-app',34666666481)]:
 ...
 receipt={'status':'PASS','utc':datetime.datetime.now(datetime.timezone.utc).isoformat(),'source':expected,'phase':'main','runs':[{**{k:run[k] for k in ('id','name','head_sha','status','conclusion','event','run_attempt','html_url')},'jobs':jobs}],'non_pass_steps':[s for s in steps if s['conclusion']!='success'],'conditional_skips_are_not_pass':True,'scope':'Core6b/client2aa coordinated setup privacy source promotion; exact packages and installed acceptance remain separate'}
