...
r=pathlib.Path('C:/r12-c05-setup-privacy-20260912/promotion');source=json.loads((r/'core-source-promoted.json').read_bytes());sha=source['signed_commit'];repo='Kiwunaka/pokrov-core'
def gh(path):return json.loads(subprocess.check_output(['gh','api',f'repos/{repo}/'+path]))
run=gh('actions/runs/34666759088');assert run['head_sha']==sha and run['status']=='completed' and run['conclusion']=='success'
