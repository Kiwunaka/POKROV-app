...
source_client='2aa57015783ceb3415e3be62bbf0a698729011c4'
...
with zipfile.ZipFile('C:/r12corec02/dist/android/pokrov-core.aar') as core_archive:
...
dex = '\n'.join((prior / ('android-dex-packages-' + label + '.log')).read_text(encoding='utf-8-sig') for label in ('direct','store'))
assert json.loads((prior/'android-dex-parsing.json').read_bytes())['status']=='PASS_TWO_EXACT_DEX_INPUTS_PARSED'
...
result = {'schema': 'pokrov.r12.final-android-package-audit/v1',
          'source_client': source_client, 'source_core': '6b271decead88b708e2fc03984b703b0a4e63ebd',
          ...
          'dex_namespace_check': {'patterns': patterns, 'matches': matches, 'logs': [ref(prior / ('android-dex-packages-' + label + '.log')) for label in ('direct','store')], 'identity_and_parsing': ref(prior/'android-dex-parsing.json')},
