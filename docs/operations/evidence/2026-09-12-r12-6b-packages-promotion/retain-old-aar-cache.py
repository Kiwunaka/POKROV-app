...
groups = [g for g in inventory['groups'] if not g['current_core'] and g['kind'] == 'jetified_aar']
assert len(groups) == 10
rows = [f for g in groups for f in g['files']]
remote = '/tmp/pokrov-r12-core-validation-1c8b33f6a771409a/retained-old-aar-cache-20260912'
...
with (out / 'old-aar-cache-upload.log').open('wb') as error:
    ...
    class Writer:
        ...
        def write(self, raw):
            ...
            if time.monotonic() - self.last > 10:
                ...
                (out / 'old-aar-cache-upload-progress.json').write_text(json.dumps({'status': 'UPLOADING', 'bytes': self.bytes, 'elapsed_seconds': round(time.monotonic() - started, 1), 'files': len(rows)}) + '\n', encoding='utf8')
            return len(raw)
        def flush(self):
            process.stdin.flush()
    writer = Writer()
...
(out / 'old-aar-cache-upload.json').write_text(json.dumps(uploaded, indent=2) + '\n', encoding='utf8')
...
print(json.dumps({'status':'PASS_EXACT_RETIRED_AAR_CACHE_ARCHIVE','archive':str(p),'archive_sha256':digest,'archive_bytes':p.stat().st_size,'files':len(seen),'file_bytes':sum(r['bytes'] for r in expected.values()),'local_files_deleted':False}))
'''.replace('ARCHIVE', uploaded['archive']).replace('DIGEST', uploaded['archive_sha256'])
