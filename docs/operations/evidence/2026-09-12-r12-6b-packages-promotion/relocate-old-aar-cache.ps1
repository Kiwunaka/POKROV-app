...
$taskPlan = [IO.File]::ReadAllText("$taskOut/old-aar-cache-retention-plan.json") | ConvertFrom-Json
$taskReceipt = [IO.File]::ReadAllText("$taskOut/old-aar-cache-retention.json") | ConvertFrom-Json
if ($taskReceipt.status -ne 'PASS_EXACT_RETIRED_AAR_CACHE_ARCHIVE' -or $taskReceipt.files -ne 30 -or $taskPlan.groups.Count -ne 10) { throw 'Exact archive verification missing' }
...
foreach ($taskGroup in $taskPlan.groups) {
    ...
    if ([IO.Path]::GetDirectoryName($taskDirectory) -ne $taskRoot -or [IO.Path]::GetFileName($taskDirectory) -notmatch '^[0-9a-f]{32}$' -or $taskGroup.current_core -or $taskGroup.kind -ne 'jetified_aar') { throw 'Unexpected transform scope' }
...
foreach ($taskGroup in $taskPlan.groups) {
    ...
    foreach ($taskFile in $taskGroup.files) {
        ...
        @{status='RELOCATING_VERIFIED_FILES';archive=$taskReceipt.archive;archive_sha256=$taskReceipt.archive_sha256;removed=$taskRemoved.ToArray()} | ConvertTo-Json -Depth 4 | Set-Content "$taskOut/old-aar-cache-relocation-progress.json"
    }
...
$taskResult = @{status='PASS_TEN_RETIRED_AAR_CACHES_RELOCATED';archive=$taskReceipt.archive;archive_sha256=$taskReceipt.archive_sha256;remote_host='pokrov-de';removed=$taskRemoved.ToArray();current_core_groups_unchanged=2;free_e_before=$taskBefore;free_e_after=(Get-PSDrive E).Free;utc=[DateTime]::UtcNow.ToString('o')}
$taskResult | ConvertTo-Json -Depth 4 | Set-Content "$taskOut/old-aar-cache-relocation.json"
