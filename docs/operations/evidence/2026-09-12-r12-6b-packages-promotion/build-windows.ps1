...
$taskRetention=[IO.File]::ReadAllText("$taskOut/prior-installer-retention.json")|ConvertFrom-Json
if ($taskRetention.status -ne 'PASS_PRIOR_INSTALLER_RETAINED') { throw 'Prior installer has no verified copy' }
if ((Get-FileHash -LiteralPath $taskRetention.local.path -Algorithm SHA256).Hash.ToLowerInvariant() -ne $taskRetention.local.sha256) { throw 'Previous installer changed' }
$taskRemoteHash = (& ssh -o BatchMode=yes -o ConnectTimeout=8 pokrov-de "sha256sum $($taskRetention.remote_path)")
if ($LASTEXITCODE -ne 0 -or -not $taskRemoteHash.StartsWith($taskRetention.remote_sha256+' ')) { throw 'Previous installer remote verification failed' }
...
$taskCacheDestination=[IO.Path]::GetFullPath('E:/r12-c05-setup-prior-flutter-cache-20260912')
if ($taskCacheSource -ne 'E:\r12client\apps\windows_shell\.dart_tool\flutter_build' -or $taskCacheDestination -ne 'E:\r12-c05-setup-prior-flutter-cache-20260912') { throw 'Cache scope mismatch' }
