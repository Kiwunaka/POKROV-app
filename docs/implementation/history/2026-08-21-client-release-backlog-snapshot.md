# Retained client release backlog snapshot
...
The 1.2.0 working source now keeps the Android foreground and lockscreen
notification private and generic, migrates retained notification-detail
preferences to false, and validates managed/native TUN MTU as `1280..1500`
with a `1280` default and usable physical-interface ceiling. Android JVM,
managed-profile and widget tests prove the source contract. OEM lockscreen,
physical-network, signed release-build and exact-candidate behavior remain
manual gates and are not release passes.

The same working source now has one generation-fenced VPN-session task scope
for Core status and selected-egress work, plus an Activity-owned bounded scope
for latency, variant, installed-app and verified-update jobs. Stop, replacement,
service destruction and Flutter-engine cleanup fence late callbacks and cancel
their owned work. Live rates and totals use Core `CommandStatus` TUN counters,
not whole-UID `TrafficStats`, and preserve explicit unavailable, warming,
reset and overflow states. Android JVM `155/155`, runtime-engine `52/52` and
app-shell `325/325` pass locally; runtime-engine also records one exact-Core
backtest as skipped without its external artifact. Both affected Flutter
packages analyze clean. Real process-kill/restart, endurance and exact-candidate
device evidence remain manual and are not implied by these source checks.

The Android updater boundary is also locally implemented for 1.2.0. Explicit
`direct` and `store` flavors retain one package ID but split authority: only
direct owns install-packages permission, private APK cache, `FileProvider` and
installer intents; store contains no direct updater surface and opens the
explicit Google Play listing. Direct validates canonical metadata, exact
size/SHA-256, package, requested version, monotonic `versionCode`, minimum SDK,
ABI and installed-to-target signer lineage before cache commit and repeats the
full check before installer handoff. Both real debug APKs were inspected; the
store manifest and updater bytecode had no direct installer path. Android JVM
passes `161/161` on each flavor, app-shell `326/326`, Android-shell `8/8`, and
both Flutter analyzers are clean. Production signing, same-signer release
upgrade/downgrade, physical-device, Play-console and exact-candidate gates are
still open and are not converted into passes by these local checks.
...
- on the exact Windows candidate, verify machine-wide service install/upgrade,
  close-to-tray/reopen while connected, tray `Выход`, UI crash while Core is
  running, service stop/restart and TUN/route/DNS restoration; source-level
  lifecycle ordering, durable recovery journal, owned-adapter snapshot/restore,
  injected rollback faults and a separate bounded privileged breadcrumb
  journal are complete, while live network and exact-artifact proof remain
  manual
