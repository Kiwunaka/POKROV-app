...
| 4 | Strict-v2 candidate metadata | `MISSING` | Generate a new handoff only after support-pin merge, trusted Windows signing and replacement artifact assembly. |
...
| 8 | Windows trust and clean-host recovery | `BLOCKED_BY_ACCESS` | No valid private Code Signing certificate exists in CurrentUser/My or LocalMachine/My; obtain one, then run trusted signing plus clean VM TUN/DNS/egress/crash/reboot/rollback. |
...
2. Obtain/install a trusted Windows Code Signing certificate.
3. Freeze the replacement four-repository tuple and rebuild exact artifacts.
4. Generate strict-v2 metadata and run exact Android/Windows manual gates.
5. Request separate authority for runtime support-key deployment, candidate
