...
|---:|---|---|---|
| 1 | Clean platform, client, Core and release-index revisions | `IN_PROGRESS_LOCAL_FREEZE; PROMOTION_BLOCKED_BY_ACTIONS_BILLING` | The prior 15/15 locally gated tuple remains Core `f44dbe8…f90d`, platform `e5ef03a…11db` and client `c196dff…1f61`. Working corrections are platform `34d1551…f435c` and client `51f41c6…dfa7b`; focused contracts and cross-repository seed validation pass, while the full aggregate gate still needs the exact tuple. Hosted PR checks remain zero-step billing failures; release-index `32f560d…2d4a` is unchanged. |
...
| 6 | Android physical-device matrix | `PASS_4045_DISCONNECTED_HOST_TRUTH; FAIL_4045_PREDEPLOY_ANDROID_ACTIVATION; MANUAL_OWNER_TEST` | Build 4045 starts honestly disconnected on Huawei and LDPlayer, closing the stale-running false-green regression. A physical connect control started the POKROV service but no Android VPN transport appeared after about 25 seconds; DNS/HTTPS were not run and cleanup restored Hiddify with Wi-Fi disabled. Deploy the authorized platform source, prove an app-owned TUN plus AWG2 handshake and egress, then AWG3.1; run Huawei TUN/DNS/egress, WARP, per-app, handoff and endurance on exact bytes. |
