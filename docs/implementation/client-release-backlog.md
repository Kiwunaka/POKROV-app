...
| 6 | Android physical-device matrix | `PASS_4045_DISCONNECTED_HOST_TRUTH; MANUAL_OWNER_TEST` | Build 4045 starts on Huawei and LDPlayer without an app-owned VPN service and reports `Подключить` / `Не защищено`, closing the stale-running false-green regression. Deploy the authorized platform source, prove an app-owned TUN plus AWG2 handshake and egress, then AWG3.1; run Huawei TUN/DNS/egress, WARP, per-app, handoff and endurance on exact bytes. |
