...
- the active local pre-candidate runtime has exact platform bindings: Android uses corrected Core commit `54e76bbb61f79bd0eb0cdbddb71f52e12645fa13`, which preserves the `egress` event subsystem, resolves AWG endpoint probe hostnames through the Core DNS router and protects only the AWG outer socket on Android, while Windows temporarily retains exact bytes from `344b317a7a09eca7943a93866b193553538bd8f6`; promotion is blocked until the platform artifacts converge on one Core source
...
`54e76bbb61f79bd0eb0cdbddb71f52e12645fa13`, and retained Windows commit
