...
- generated Windows profiles follow the proven POKROV Core/Hiddify system-TUN shape: `address`, `stack: system`, `strict_route`, route-level `sniff` and `hijack-dns`, typed TCP/UDP DNS servers, and no legacy `dns-out` or TUN-level sniff/NAT fields. `Full tunnel` removes inherited Internet bypass rules while retaining local/private LAN access; selected and excluded process modes use opposite route and DNS decisions as their labels promise
- Windows connect verification is two-stage. The loopback mixed-port request proves that Core and the selected outbound work; a second forced-direct request from the normal Windows socket path proves TUN capture and DNS. The UI may enter `running` only after both succeed. A failed system-path probe stops Core and reports `desktop_tun_egress_probe_failed` instead of showing a false protected state
...
- the regular Windows shell stays unelevated; the first TUN connect checks the process token and, when required, relaunches the exact executable through the Windows `runas` prompt with an internal `--connect` continuation. Cancelled or unavailable elevation stops before Core start and cannot be presented as a working VPN
