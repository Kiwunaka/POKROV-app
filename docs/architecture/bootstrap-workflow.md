...
- the Android diagnostics story is stronger than the original seed: the shared shell exposes `Runtime health` controls for refresh, runtime priming, local smoke-profile staging, and live connect or disconnect, so bootstrap failures are easier to classify before the physical-device gate
