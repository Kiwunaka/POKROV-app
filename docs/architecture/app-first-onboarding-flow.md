...
- the visible distinction between `Все, кроме РФ` and `Полный режим` therefore remains a client `Правила` choice until the backend grows a dedicated device-behavior field
