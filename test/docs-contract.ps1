...
if ($developmentTarget.product_version -ne '1.2.0' -or
    [int]$developmentTarget.platform_build -ne 36 -or
    $developmentTarget.package_version -ne '1.2.0+36' -or
    $developmentTarget.state -ne 'PRE_CANDIDATE_LOCAL' -or
    $developmentTarget.candidate_created -ne $false) {
  $errors += 'Release handoff development target must remain exact uncreated 1.2.0+36 pre-candidate truth'
}
...
foreach ($currentFact in @('`1.1.6`', '`1.2.0+36`', '`PRE_CANDIDATE_LOCAL`', '| Candidate created | `false` |')) {
