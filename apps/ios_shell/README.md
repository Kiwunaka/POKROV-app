...
- `PokrovCore.xcframework` must be built on macOS from matching Core source that exposes `LibboxTransportCapabilities`, before this host can compile or claim runtime proof. The gomobile output is static: Runner and PacketTunnelExtension link it separately; it is not copied as a runtime framework. Runner's executable path is locator metadata, not proof of Core identity or VPN health.
