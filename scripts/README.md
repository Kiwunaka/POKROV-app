...
- `sync-pokrov-core-runtime.ps1` copies only the retained POKROV Core 1.0.0 Android and Windows artifact identities from the separate core checkout after verifying commit, version, sizes, and SHA-256 values. It explicitly rejects the different clean-tag rebuild; replacing the pinned candidate requires a new patch release and refreshed evidence.
