...
- `build-android-production.ps1` loads that local identity only into the current process, builds the canonical direct release APK, rejects Android Debug signing, matches the signer fingerprint, and writes ignored candidate-local signing evidence beside the APK.
